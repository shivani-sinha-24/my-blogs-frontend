...
const BlogPage = ({reload,setReload,isUserLoggedin}) => {
  ...
  useEffect(()=>{
    axios.get(`https://blog-be-kappa.vercel.app/blog/${blogId}`)
    .then(res=>{
  ...
  const addCmnt= () =>{
    setReload(false);
    if (comntInput) {
      axios.post(`https://blog-be-kappa.vercel.app/blog/add-comment`, blogDetail)
      .then((res) => {
    ...
    <div className="card all-side-shadow">
      ...
      <div className="card-body text-center">   
        <div className="text-center">
          {/* {blog?.image&&<img src={`https://blog-be-kappa.vercel.app/uploads/${blog?.image}`} className="card-img-top blog-image" alt="..."/>} */}
